...
p1 <- ggplot(wealth_long %>% dplyr::filter(Real_Nominal == "Nominal"),
  ...
  labs(
    title = "Wealth Evolution: 2004 Leaders - Optimized Portfolios vs S&P 500 Benchmark",
...
p2 <- ggplot(wealth_long %>% dplyr::filter(Real_Nominal == "Real (CPI-Adjusted)"),
  ...
  scale_y_continuous(breaks = scales::pretty_breaks(n = 8)) +
  labs(
    title = "Real Wealth Evolution: 2004 Leaders - Optimized Portfolios vs S&P 500 Benchmark",
...
output_prefix <- "portfolio_2004_leaders_"
